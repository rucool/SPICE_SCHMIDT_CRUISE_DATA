...
arg_parser = argparse.ArgumentParser(description='Detect thermohaline staircases in glider profiles and save hovmoller figures',
...
if not _target:  # do not overwrite current track CSV during backfill
    prof_coords[["lat", "lon", "profile_time"]].rename(columns={"profile_time": "time"}).to_csv(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), f"{glider}_latest_track.csv"), index=False)
...
plot_vars_list = ["CT", "ml_height", "turner", "sigma", "classification", "counts", "depth_range"]
for v in plot_vars_list:
    os.makedirs(os.path.join(daily_dir, f"{glider}_{v}"), exist_ok=True)
...
plt.savefig(os.path.join(daily_dir, f'{glider}_CT', f'{glider}_CT_{run_ts}.png'), dpi=200, bbox_inches='tight')
plt.show()
...
add_station_markers(ax, fig)
plt.gcf().canvas.draw()  # force full render before tight-bbox crop
plt.savefig(os.path.join(daily_dir, f'{glider}_ml_height', f'{glider}_ml_height_{run_ts}.png'), dpi=200, bbox_inches='tight')
plt.show()
...
add_station_markers(ax, fig)
plt.gcf().canvas.draw()  # force full render before tight-bbox crop
plt.savefig(os.path.join(daily_dir, f'{glider}_turner', f'{glider}_turner_{run_ts}.png'), dpi=200, bbox_inches='tight')
plt.show()
...
add_station_markers(ax, fig)
plt.gcf().canvas.draw()  # force full render before tight-bbox crop
plt.savefig(os.path.join(daily_dir, f'{glider}_sigma', f'{glider}_sigma_{run_ts}.png'), dpi=200, bbox_inches='tight')
plt.show()
...
plt.gcf().canvas.draw()  # force full render before tight-bbox crop
plt.savefig(os.path.join(daily_dir, f'{glider}_classification', f'{glider}_classification_{run_ts}.png'), dpi=200, bbox_inches='tight')
plt.show()
...
plt.gcf().canvas.draw()  # force full render before tight-bbox crop
plt.savefig(os.path.join(daily_dir, f'{glider}_depth_range', f'{glider}_depth_range_{run_ts}.png'), dpi=200, bbox_inches='tight')
plt.show()
