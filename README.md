...
- **`falkor_ctd_staircase.py`** — runs the same staircase-detection pipeline as the glider scripts, but against shipboard CTD casts (Sea-Bird SBE9 `.cnv` files, both up- and down-casts) collected during the R/V Falkor (too) cruise. Reads whichever local directory of `.cnv` files it's pointed at; writes staircase-results CSVs plus the same 7 survey figures ru29/glider scripts produce, plotted against longitude and always split into down-cast/up-cast panels, since a stationary CTD cast doesn't spatially separate its two directions the way a moving glider yo does. Reuses `configs/cruise_stations.yml` and `configs/staircase_vars.yml` for the station-marker legend and colorbar limits.
